#!/usr/bin/env bash
# Deploys MezoCirclesVault to Mezo testnet.
# Usage:
#   1. cp .env.deploy.example .env.deploy
#   2. Edit .env.deploy — fill MEZOCIRCLES_VAULT_OWNER + DEPLOYER_PRIVATE_KEY
...
: "${MEZOCIRCLES_VAULT_OWNER:?missing in .env.deploy}"
...
echo "▶ MUSD:               $MEZO_MUSD"
echo "▶ Vault owner:        $MEZOCIRCLES_VAULT_OWNER"
...
if [ "$BROADCAST_FLAG" = "--broadcast" ]; then
  ...
  forge script script/DeployMezoCirclesVault.s.sol:DeployMezoCirclesVault \
    --rpc-url "$RPC_URL" \
